...
Additional metrics such as energy error are incorporated into the fitness function.
Parallel processing is used to evaluate the simulation candidates concurrently.
"""
...
# Define fitness target parameters
TARGET_SLOPE = -5.0
...
def default_json_converter(o):
    ...
    Converts numpy arrays to lists and numpy scalars to native Python types.
    """
    if hasattr(o, "tolist"):
        return o.tolist()
...
def random_candidate(param_ranges):
    """Generate a random candidate parameter set within given ranges."""
    return {k: np.random.uniform(v[0], v[1]) for k, v in param_ranges.items()}

def mutate(candidate, param_ranges, mutation_rate=0.1):
    """
    Mutate a candidate parameter set.
    Each parameter has a chance to be perturbed by up to 10% of its range.
    """
...
def crossover(parent1, parent2):
    """
    Perform crossover between two parent candidate parameter sets.
...
def evaluate_candidate(candidate, base_params, num_cycles_test=1):
    """
    Evaluate a candidate by running a short simulation.
    Uses num_cycles_test to reduce computation time.
    
    Returns:
        A tuple: (candidate, fitness, slope, energy_error)
    """
    # Combine base parameters with candidate parameters
    sim_params = base_params.copy()
    sim_params.update(candidate)
    # Use a shorter simulation for evaluation
    ...
    try:
        ...
        if slope is None:
            ...
            fitness_val = -abs(slope - TARGET_SLOPE) - ALPHA_ENERGY * energy_error
    except Exception as e:
        print(f"[ERROR] Simulation failed for candidate {candidate}: {e}")
        slope = None
        energy_error = None
        fitness_val = -1e6
    return (candidate, fitness_val, slope, energy_error)

def genetic_algorithm_optimization(generations=5, population_size=10, base_params=None, initial_param_ranges=None):
    """
    Perform parameter optimization using a genetic algorithm.
    
    Returns a dictionary with optimization results and archives all runs.
    """
    ...
    for gen in range(generations):
        ...
        print(f"[INFO] Generation {gen+1} results saved in {csv_file}")
        
        # Select top candidates for next generation (elitism)
    ...
    # Final best candidate evaluation with full simulation parameters
    ...
    best_overall = {
        ...
        "final_result": "Excluded from JSON summary"  # Exclude full simulation arrays
    }
