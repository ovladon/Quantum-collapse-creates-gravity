...
import numpy as np
import matplotlib
# Use headless backend for parameter sweep
matplotlib.use('Agg')
...
def run_field_simulation(
    ...
    The field evolves according to a modified Klein-Gordon equation with collapse terms:
      (φ_{t+1} - 2φ_t + φ_{t-1})/dt^2 = ∇²φ_t - m^2 φ_t - collapse_rate*(φ_t - φ_avg) + sqrt(collapse_rate)*η_t,
    with periodic boundary conditions.
    After evolving for T = steps_per_cycle * num_cycles steps, the energy density is computed,
    and the gravitational potential is obtained by solving ∇²Φ = 4πG ρ.
    The function returns the noise exponent (slope) of the azimuthally averaged power spectrum of Φ.
    """
    ...
    # Modified Klein-Gordon with collapse and noise:
    # (φ^{n+1} - 2φ^n + φ^{n-1})/dt^2 = ∇²φ^n - m^2 φ^n - collapse_rate*(φ^n - φ_avg) + sqrt(collapse_rate)*η^n.
    for t in range(T):
        phi_avg = np.mean(phi)
    ...
    # Compute energy density ρ as effective mass density:
    ...
    # Solve Poisson equation for gravitational potential Φ in 2D:
    phi_grav = solve_poisson(energy_density, dx, G, relativistic_factor=relativistic_factor)
    
    # Compute power spectrum of Φ.
...
def param_sweep():
    ...
    for each combination, and saves the noise spectrum slope (noise exponent) to a CSV file.
    Also produces a DOCX summary report.
    """
    from docx import Document
    from docx.shared import Pt
    ...
    discussion = (
        "This parameter sweep systematically explores a range of collapse dynamics parameters in a relativistic-inspired simulation "
        "of a real scalar field. The simulation couples collapse-like dynamics with gravitational potential computation via the Poisson "
        "equation (with a crude relativistic correction). The emergent noise spectrum of the gravitational potential is analyzed, and its power-law "
        "exponent is estimated. A steep negative exponent (close to -5) indicates strong suppression of small-scale fluctuations, suggesting that the "
        "gravitational field is dominated by large-scale coherence. These quantitative results provide an independent signature that can be compared "
        "with experimental observations."
    )
    doc.add_paragraph(discussion)
    doc.add_heading("Next Steps", level=1)
    next_steps = (
        "1) Identify parameter combinations that consistently yield noise exponents near -5.\n"
        "2) Increase simulation resolution and duration for robust statistics.\n"
        "3) Refine the collapse model to more accurately capture CSL/GRW dynamics and include further relativistic corrections.\n"
        "4) Run control simulations with independently generated potentials to guard against circular reasoning.\n"
        "5) Compare the predicted noise spectrum with experimental data from short-range gravity experiments or gravitational-wave detectors.\n"
    )
...
def run_visualization_simulation():
    ...
    """
    interactive_mode = True  # Set True if you want to see the GUI
    G = 1.0
    L = 10.0
